...
use holochain::sweettest::{SweetAgents, SweetAppBatch, SweetCell, SweetConductor, SweetConductorBatch, SweetDnaFile};
...
pub async fn test_create_trust_atom() {
    ...
    let target_hash: EntryHash = conductor.call(&cell1.zome("trust_atom"), "create_string_target", "Nuka Sushi").await;
    ...
    let extra: BTreeMap<String, String> = BTreeMap::from([(
        "details".into(),
        "Excellent specials. The regular menu is so-so. Their coconut curry (special) is to die for".into(),
    )]);
    ...
    let _result: trust_atom_types::TrustAtom = conductor.call(&cell1.zome("trust_atom"), "create_trust_atom", trust_atom_input).await;
    ...
    let forward_links: Vec<Link> = conductor
        .call(&cell1.zome("trust_atom"), "test_helper_list_links_for_base", agent_address)
        .await;
    ...
    let expected_link_tag_string = format!(
        "{}{}{}{}{}{}{}{}{}",
        "Ŧ", "→", "sushi", unicode_nul, ".800000000", unicode_nul, bucket, unicode_nul, expected_entry_hash
    );
    ...
    let backward_links: Vec<Link> = conductor
        .call(&cell1.zome("trust_atom"), "test_helper_list_links_for_base", target_hash.clone())
        .await;
...
pub async fn test_query_mine() {
    ...
    let target_hash: EntryHash = conductor.call(&cell1.zome("trust_atom"), "create_string_target", "Sushi Ran").await;
...
pub async fn test_query_mine_with_content_full() {
    let (conductor, _agent, cell1): (SweetConductor, AgentPubKey, SweetCell) = setup_1_conductor().await;
...
pub async fn test_get_extra() {
    ...
    let target_hash = conductor.call(&cell1.zome("trust_atom"), "create_string_target", "Nuka Sushi").await;
    ...
    let mock_input = trust_atom_types::TrustAtomInput {
        ...
        extra: Some(BTreeMap::from([
            ("extra_stuff".to_string(), "Say some extra stuff here".to_string()),
            ("another_thing".to_string(), "Put more information here".to_string()),
        ])),
    };

    let _mock_trust_atom: trust_atom_types::TrustAtom = conductor.call(&cell1.zome("trust_atom"), "create_trust_atom", mock_input.clone()).await;

    let mock_entry = trust_atom_integrity::Extra {
        fields: mock_input.extra.unwrap(),
    };
    let mock_extra_entry_hash: EntryHash = conductor.call(&cell1.zome("trust_atom"), "calc_extra_hash", mock_entry).await;

    let mock_extra_data: trust_atom_integrity::Extra = conductor.call(&cell1.zome("trust_atom"), "get_extra", mock_extra_entry_hash).await;
...
pub async fn test_get_entry_by_actionhash() {
    ...
    let action_hash: ActionHash = conductor.call(&cell1.zome("trust_atom"), "create_test_entry", test_entry).await;

    let retrieval: trust_atom_integrity::Example = conductor.call(&cell1.zome("trust_atom"), "test_get_entry_by_action", action_hash).await;
...
pub async fn setup_conductors(n: usize) -> (SweetConductorBatch, Vec<AgentPubKey>, SweetAppBatch) {
    ...
    let all_agents1: Vec<holochain::core::AgentPubKey> = future::join_all(conductors.iter().map(|c| SweetAgents::one(c.keystore()))).await;
    ...
    let apps = conductors.setup_app_for_zipped_agents("app", &all_agents1, &[dna]).await.unwrap();
