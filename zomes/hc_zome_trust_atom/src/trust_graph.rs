...
fn build_rollup_silver(
  ...
  for target in targets.clone() {
    ...
    if &target != me && !agents.contains(&target) {
      let trust_atom_latest = get_latest_trust_atom(target)
...
fn get_latest_trust_atom(
  base: EntryHash,
  target: EntryHash,
  tag_filter: Option<LinkTag>,
  link_direction: Option<LinkDirection>
) -> ExternResult<Option<TrustAtom>> {
  let link_latest = get_latest_link(base, target, tag_filter)?;
  if let Some(latest) = link_latest {
    let trust_atom_latest = convert_link_to_trust_atom(link_latest, &link_direction, &base)?;
    Ok(latest)
  }
  else {
    Ok(None)
